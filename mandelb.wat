(module
  ...
  (func (export "calc")
    ...
    (local.set $hw (f64.mul (f64.convert_i32_u (local.get $width)) (f64.const 0.5)))
    (local.set $hh (f64.mul (f64.convert_i32_u (local.get $height)) (f64.const 0.5)))
    (local.set $cx (f64.sub (local.get $cx) (f64.mul (local.get $hw) (local.get $cs))))
    (local.set $cy (f64.sub (local.get $cy) (f64.mul (local.get $hh) (local.get $cs))))
    (local.set $zx (f64.sub (local.get $zx) (f64.mul (local.get $hw) (local.get $zs))))
    (local.set $zy (f64.sub (local.get $zy) (f64.mul (local.get $hh) (local.get $zs))))

    (local.tee $row_len (i32.shl (local.get $width) (i32.const 3)))
    (local.set $grid_len (i32.mul (local.get $height)))

    (local.set $cxi (f64.mul (local.get $cs) (f64.convert_i32_u (global.get $count))))
    (local.set $cxj (f64.mul (local.get $cs) (f64.convert_i32_u (local.get $width))))
    (local.set $zxi (f64.mul (local.get $zs) (f64.convert_i32_u (global.get $count))))
    (local.set $zxj (f64.mul (local.get $zs) (f64.convert_i32_u (local.get $width))))

    (local.set $cx (f64.add (local.get $cx) (f64.mul (local.get $cs) (f64.convert_i32_u (global.get $id)))))
    (local.set $zx (f64.add (local.get $zx) (f64.mul (local.get $zs) (f64.convert_i32_u (global.get $id)))))
    (loop $y
      (local.tee $j (i32.add (local.get $j) (local.get $row_len)))
      (loop $x
        (f64.store (local.get $i) (call $calc_point (local.get $cx) (local.get $cy) (local.get $zx) (local.get $zy)))

        (local.set $cx (f64.add (local.get $cx) (local.get $cxi)))
        (local.set $zx (f64.add (local.get $zx) (local.get $zxi)))
        (local.tee $i (i32.add (local.get $i) (local.get $chunk_len)))
        (br_if $x (i32.lt_u (local.get $j)))
      )
      (local.set $cx (f64.sub (local.get $cx) (local.get $cxj)))
      (local.set $cy (f64.add (local.get $cy) (local.get $cs)))
      (local.set $zx (f64.sub (local.get $zx) (local.get $zxj)))
      (local.set $zy (f64.add (local.get $zy) (local.get $zs)))
