  ...
  (func $calc_point (param $cx f64) (param $cy f64) (param $zx f64) (param $zy f64) (result f64 f64)
    ...
    (block $skip (loop $cont
      ...
      (if (i32.or (f64.ne (local.get $mx) (local.get $mx)) (f64.ne (local.get $my) (local.get $my)))
        (then
          (local.set $mx (f64.const 0))
          (local.set $my (f64.const 0))
          (local.set $sx (f64.const 1e100))
          (local.set $sy (f64.const 1e100))
        )
        (else
      ...
      (if (f64.lt (f64.add) (global.get $escape2))
        ...
        (else
          ...
          (br_if $skip (f64.lt (f64.add (f64.abs (local.get $mx)) (f64.abs (local.get $my))) (global.get $epsilon)))
        )
      )
    ...
    (if (result f64 f64) (local.get $o)
      (then
        (f64.sqrt (f64.add (f64.mul (local.get $sx) (local.get $sx)) (f64.mul (local.get $sy) (local.get $sy))))
